...
#include "bumperbot_planning/astar_planner_plugin.hpp"
...
{   
    ...
    pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>("/goal_pose", 10,
                                                                        std::bind(&AStarPlanner::goalCallback, this, _1));

    // Initialize Publishers
    path_pub_ = create_publisher<nav_msgs::msg::Path>("/astar/path", 10);
    map_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>("/astar/visited_map", 10);
...
{
    RCLCPP_INFO_STREAM(node_->get_logger(), "Cleaning up plugin " << name_.c_str() << " of type A-Star Planner");
}

void AStarPlanner::activate()
{
    RCLCPP_INFO_STREAM(node_->get_logger(), "Activating the plugin " << name_.c_str() << " of type A-Star Planner");   
}

void AStarPlanner::deactivate()
{
    RCLCPP_INFO_STREAM(node_->get_logger(), "Deactivating the plugin " << name_.c_str() << " of type A-Star Planner");   
}
...
nav_msgs::msg::Path AStarPlanner::createPlan(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal)
{
    ...
    start_node.heurestic = getHeurestic(start_node, goal_node);
    //start_node.total_cost = start_node.cost + start_node.heurestic;
...
float AStarPlanner::getHeurestic(const GraphNode& node, const GraphNode& goal_node)
{
    return abs(node.x_ - goal_node.x_) + abs(node.y_ - goal_node.y_);
}
