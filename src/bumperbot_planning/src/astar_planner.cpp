...
geometry_msgs::msg::Pose AStarPlanner::gridtoWorld(const GraphNode& node)
{
    ...
    pose.position.x = node.x_ * map_->info.resolution + map_->info.origin.position.x;
    pose.position.y = node.y_ * map_->info.resolution + map_->info.origin.position.y;
    return pose;
}

bool AStarPlanner::poseOnMap(const GraphNode& node)
{
    return node.x_ >= 0 && node.x_ < static_cast<int>(map_->info.width) && 
            node.y_ >= 0 && node.y_ < static_cast<int>(map_->info.height);
}

unsigned int AStarPlanner::poseToCell(const GraphNode& node)
{
    return node.y_ * map_->info.width + node.x_;
}
